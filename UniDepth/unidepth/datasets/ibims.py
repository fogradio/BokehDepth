"""IBIMS-1 dataset loader with optional manifest + defocus stack support."""
...
IBIMS_MIN_DEPTH = 0.005
IBIMS_MAX_DEPTH = 25.0
IBIMS_DEPTH_SCALE = 50.0 / 65535.0  # depth_png * scale -> metres


class IBims(ImageDataset):
    min_depth = IBIMS_MIN_DEPTH
    max_depth = IBIMS_MAX_DEPTH
    depth_scale = 1000.0
    ...
    def load_dataset(self):
        ...
        txt_string = txt_file.tobytes().decode("ascii")[:-1]
    ...
    def _load_manifest_dataset(self, manifest_path: str) -> None:
        ...
        if not records:
            raise FileNotFoundError(
                f"Parsed manifest {manifest_path} but no valid IBIMS samples were found."
            )
    ...
    def _build_manifest_sample(self, entry: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        mask_paths: List[str] = []
        for key in ("mask", "masks", "mask_list", "source_mask", "mask_invalid", "mask_transp"):
            value = entry.get(key)
            if not value:
                continue
        ...
        intrinsics_path = entry.get("source_intrinsics") or entry.get("intrinsics")
        if intrinsics_path:
    ...
    def pre_pipeline(self, results):
        ...
        copies = self.num_copies * getattr(self, "num_frames", 1)
        results["dense"] = [True] * copies
        results["quality"] = [1] * copies
        return results
    ...
    def _get_single_item_from_manifest(self, idx: int):
        ...
        if not np.count_nonzero(valid_mask):
            raise ValueError(f"IBIMS depth map {sample['depth_path']} has no valid pixels.")
        ...
        if K is None:
            ...
            warnings.warn(
                f"[IBIMS] Missing intrinsics for {sample.get('image_path', 'unknown')}. "
    ...
    def _load_manifest_depth(self, sample: Dict[str, Any]) -> np.ndarray:
        ...
        if depth.ndim == 3:
            depth = depth[..., 0]
        depth *= IBIMS_DEPTH_SCALE if depth.max() > 1.0 else 1.0
    ...
    def _get_manifest_intrinsics(self, sample: Dict[str, Any]) -> Optional[torch.Tensor]:
        ...
        if intrinsics_path:
            cached = self._intrinsics_cache.get(intrinsics_path)
            if cached is None and os.path.isfile(intrinsics_path):
                matrix = self._load_intrinsics_file(intrinsics_path)
                if matrix is not None:
                    cached = torch.from_numpy(matrix.astype(np.float32))
                    self._intrinsics_cache[intrinsics_path] = cached
            if cached is not None:
                return cached.clone()

        matrix = sample.get("intrinsics_matrix")
        if matrix is not None:
            return torch.from_numpy(np.asarray(matrix, dtype=np.float32)).clone()
    ...
    def _load_intrinsics_file(self, path: str) -> Optional[np.ndarray]:
        ...
        if not content:
            return None
        tokens = [tok.strip() for tok in content.replace(";", ",").split(",") if tok.strip()]
        if len(tokens) != 4:
            return None
        fx, fy, cx, cy = (float(tok) for tok in tokens)
        matrix = np.array(
            [
                [fx, 0.0, cx],
                [0.0, fy, cy],
