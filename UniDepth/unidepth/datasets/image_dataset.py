...
class ImageDataset(BaseDataset):
        ...
        with tables.File(
            ...
            for key_mapper, idx_mapper in mapper.items():
                ...
                if "image" in key_mapper:
                    ...
                    results[(0, 0)]["camera"] = batch_camera
    ...
    def preprocess(self, results):
        ...
        for i, seq in enumerate(results["sequence_fields"]):
            ...
            original_shape = seq_data["image"].shape[-2:]
            original_K = seq_data["camera"].K.clone()
            results[seq] = self.resizer(results[seq])
            seq_data = results[seq]
            if getattr(self, "validate_intrinsics", False):
                new_h, new_w = seq_data["image"].shape[-2:]
                scale_x = new_w / float(original_shape[1])
                scale_y = new_h / float(original_shape[0])
                K_new = seq_data["camera"].K
                expected_fx = original_K[..., 0, 0] * scale_x
                expected_fy = original_K[..., 1, 1] * scale_y
                if not torch.allclose(
                    ...
                    raise AssertionError(
                        "fx scaling mismatch after resize: "
                        f"expected ~{expected_fx.mean().item():.4f}, "
                        f"got {K_new[..., 0, 0].mean().item():.4f}"
                    )
                if not torch.allclose(
                    K_new[..., 1, 1],
                    expected_fy,
                    ...
                    raise AssertionError(
                        "fy scaling mismatch after resize: "
                        f"expected ~{expected_fy.mean().item():.4f}, "
                        f"got {K_new[..., 1, 1].mean().item():.4f}"
                    )
            seq_data.setdefault("camera_fields", set()).add("K")
            seq_data["K"] = seq_data["camera"].K.clone()
    ...
    def __getitem__(self, idx, retries=0):
        ...
        try:
            ...
            return self.__getitem__(new_idx, retries=retries + 1)
        return results
