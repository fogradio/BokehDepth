...
def collate_fn(in_data: List[List[Dict[str, Any]]], is_batched: bool = True):
    ...
    if is_batched:
        if isinstance(in_data, (list, tuple)):
            if in_data and not isinstance(in_data[0], dict):
                in_data = in_data[0]
        else:
            in_data = [in_data]
    ...
    for x in in_data:  # here iter over batches
        ...
        # === Debug: check whether the data contains a camera field ===
        if len(out_data) == 0 and DEBUG_LOG_ENABLED:  # only print for the first sample
            print(f"[collate_fn debug] Keys in sample: {list(x.keys())}")
            if "camera" in x:
                camera_obj = list(x['camera'].values())[0] if x['camera'] else None
                print(f"[collate_fn debug] camera type: {type(camera_obj)}")
                print(f"[collate_fn debug] camera object: {camera_obj}")
            else:
                print("[collate_fn debug] WARNING: 'camera' not found in sample!")
        for k, v in x.items():
            if "img_metas" not in k:
                ...
                # Save depth padding info.
                v["depth_paddings"] = padding
                # Save geometric-transform parameters to support the focus-stack invariance loss.
                # Pull these parameters from x (they should have been set in preprocess/augment).
                if "flip" in x:
                    v["flip"] = x["flip"]
                if "flip_direction" in x:
                    v["flip_direction"] = x["flip_direction"]
                if "scale_factor" in x:
                    v["scale_factor"] = x["scale_factor"]
                if "paddings" in x:
                    v["paddings"] = x["paddings"]
                if "resized_shape" in x:
                    v["resized_shape"] = x["resized_shape"]
                if "image_ori_shape" in x:
                    v["image_ori_shape"] = x["image_ori_shape"]
                img_metas.append(v)
        ...
        if isinstance(dataset, ImageDataset):
            ...
            for idx, data in enumerate(dataset.dataset):
                ...
                if isinstance(data, dict):
                    num = data.get("num_samples")

                if num is None and hasattr(dataset, "sequences"):
                    try:
                        sequence = dataset.sequences[idx]
                        if isinstance(sequence, dict) and "image" in sequence:
                            num = len(sequence["image"])
                    except Exception:
                        num = None

                if num is None:
                    # Fallback to counting the entry itself when metadata is missing.
                    num = 1

                numerator.append(int(num))

            total_num_samples = sum(numerator)
            if total_num_samples == 0:
                total_num_samples = len(dataset)
                numerator = [1] * len(dataset)
