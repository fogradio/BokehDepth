...
def _make_dinov2_model(
    ...
    **kwargs,
    ...
    if pretrained == "":
        ...
        try:
            state_dict = torch.load(pretrained, **load_kwargs)
        except (pickle.UnpicklingError, RuntimeError) as err:
            err_msg = str(err)
            should_retry = isinstance(err, pickle.UnpicklingError) or "Weights only load failed" in err_msg
            if should_retry:
                safe_globals = getattr(torch.serialization, "add_safe_globals", None)
                if callable(safe_globals):
                    scalar_cls = getattr(getattr(getattr(np, "_core", None), "multiarray", None), "scalar", None)
                    extras = [scalar_cls] if scalar_cls is not None else []
                    if extras:
                        try:
                            safe_globals(extras)
                        except Exception:
