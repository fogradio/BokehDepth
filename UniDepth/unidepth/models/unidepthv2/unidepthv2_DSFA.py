...
from unidepth.models.encoder_DSFA import dinov2_vitl14_DSFA
...
class UniDepthV2DSFA(
    ...
    def __init__(
        ...
        config,
        eps: float = 1e-6,
        debug: bool = False,  # debug switch, off by default
        **kwargs,
    ):
        super().__init__()
        self.eps = eps
    ...
    def infer(
        ...
        normalize=True,
        focus_stack: torch.Tensor | None = None,
        focus_k: torch.Tensor | None = None,
    ):
        ...
        if focus_stack is not None:
            if focus_stack.dim() == 4:
                focus_stack = focus_stack.unsqueeze(0)
            elif focus_stack.dim() == 3:
                focus_stack = focus_stack.unsqueeze(0).unsqueeze(0)
            elif focus_stack.dim() != 5:
                raise ValueError(
                    f"focus_stack expected 5D tensor, but got shape {tuple(focus_stack.shape)}"
                )
            if focus_stack.shape[0] != rgb.shape[0]:
                if focus_stack.shape[0] == 1:
                    focus_stack = focus_stack.expand(rgb.shape[0], -1, -1, -1, -1)
                else:
                    raise ValueError(
                        "focus_stack batch size does not match rgb batch size."
                    )
            focus_stack = focus_stack.to(self.device)
            bsz, num_frames, channels, height, width = focus_stack.shape
            focus_stack = focus_stack.reshape(bsz * num_frames, channels, height, width)
            if normalize:
                focus_stack = TF.normalize(
                    focus_stack.float() / 255.0,
                    mean=IMAGENET_DATASET_MEAN,
                    std=IMAGENET_DATASET_STD,
                )
            focus_stack = F.pad(
                focus_stack, (pad_left, pad_right, pad_top, pad_bottom), value=0.0
            )
            focus_stack = F.interpolate(
                focus_stack, size=(new_H, new_W), mode="bilinear", align_corners=False
            )
            focus_stack = focus_stack.reshape(
                bsz, num_frames, channels, new_H, new_W
            )

        if focus_k is not None and not torch.is_tensor(focus_k):
            focus_k = torch.tensor(focus_k)
    ...
    def encode_decode(self, inputs, image_metas=[]):
        ...
        # === Debug: inspect camera parameters and ray computation ===
        if self.debug and not self.training:
            print(f"\n[encode_decode debug] === entering encode_decode ===")
            print(f"[encode_decode debug] input image shape: B={B}, H={H}, W={W}")
            print(f"[encode_decode debug] 'camera' present in inputs: {'camera' in inputs}")
            print(f"[encode_decode debug] inputs.get('camera', None) is not None: {inputs.get('camera', None) is not None}")

            if inputs.get("camera", None) is not None:
                cam = inputs["camera"]
                print(f"[encode_decode debug] camera type: {type(cam)}")
                if hasattr(cam, 'K'):
                    print(f"[encode_decode debug] camera.K shape: {cam.K.shape}")
                    print(f"[encode_decode debug] camera.K[0]:\n{cam.K[0].detach().cpu()}")
                    K_sample = cam.K[0]
                    fx, fy = K_sample[0, 0].item(), K_sample[1, 1].item()
                    cx, cy = K_sample[0, 2].item(), K_sample[1, 2].item()
                    print(f"[encode_decode debug] camera intrinsics: fx={fx:.2f}, fy={fy:.2f}, cx={cx:.2f}, cy={cy:.2f}")
                    print(f"[encode_decode debug] image center: H/2={H/2:.2f}, W/2={W/2:.2f}")
                    print(f"[encode_decode debug] |cx - W/2|: {abs(cx - W/2):.2f}")
                    print(f"[encode_decode debug] |cy - H/2|: {abs(cy - H/2):.2f}")

        if inputs.get("camera", None) is not None:
            inputs["rays"] = inputs["camera"].get_rays(shapes=(B, H, W))

            # === Debug: inspect ray-computation output ===
            if self.debug and not self.training:
                rays = inputs["rays"]
                print(f"[encode_decode debug] rays computed successfully!")
                print(f"[encode_decode debug] rays shape: {rays.shape}")
                print(f"[encode_decode debug] rays dtype: {rays.dtype}")
                print(f"[encode_decode debug] rays stats: min={rays.min().item():.4f}, max={rays.max().item():.4f}, mean={rays.mean().item():.4f}")
                # Inspect each ray component.
                if rays.dim() == 4 and rays.shape[1] == 3:  # B, 3, H, W
                    rays_x, rays_y, rays_z = rays[:, 0], rays[:, 1], rays[:, 2]
                    print(f"[encode_decode debug] rays_x (horizontal) range: [{rays_x.min().item():.4f}, {rays_x.max().item():.4f}], mean={rays_x.mean().item():.4f}")
                    print(f"[encode_decode debug] rays_y (vertical) range: [{rays_y.min().item():.4f}, {rays_y.max().item():.4f}], mean={rays_y.mean().item():.4f}")
                    print(f"[encode_decode debug] rays_z (depth axis) range: [{rays_z.min().item():.4f}, {rays_z.max().item():.4f}], mean={rays_z.mean().item():.4f}")
                    print(f"[encode_decode debug] [warn] rays_z mean should be close to 1.0; a value clearly below 1.0 causes depth under-estimation")
        else:
            if self.debug and not self.training:
                print(f"[encode_decode debug] WARNING: camera is None, rays not computed!")

        focus_stack = inputs.get("focus_stack")
        if focus_stack is None:
            focus_stack = inputs.get("defocus_stack")
        if focus_stack is not None:
            if focus_stack.dim() == 4:
                focus_stack = focus_stack.unsqueeze(1)
            elif focus_stack.dim() == 3:
                focus_stack = focus_stack.unsqueeze(0).unsqueeze(0)
            elif focus_stack.dim() != 5:
                raise ValueError(
            ...
            inputs["focus_stack"] = focus_stack

        focus_k = inputs.get("focus_k")
        if focus_k is None:
            focus_k = inputs.get("defocus_k")
        if focus_k is not None:
            if focus_k.dim() == 1:
                focus_k = focus_k.unsqueeze(0)
            if (
                focus_k.dim() == 2
                and focus_stack is not None
                and focus_k.shape[0] != focus_stack.shape[0]
            ):
                focus_k = focus_k.unsqueeze(0).expand(focus_stack.shape[0], -1)
            inputs["focus_k"] = focus_k

        features, tokens = self.pixel_encoder(
            inputs["image"], focus_stack=focus_stack, focus_k=focus_k
        )
        inputs["features"] = [
        ...
        # === Debug: inspect depth output ===
        if self.debug and not self.training:
            print(f"\n[encode_decode debug] === depth computation ===")
            print(f"[encode_decode debug] outputs['rays'] shape after rearrange: {outputs['rays'].shape}")
            rays_z_after = outputs["rays"][:, -1:]  # z component
            print(f"[encode_decode debug] rays_z (used in depth computation) stats: min={rays_z_after.min().item():.4f}, max={rays_z_after.max().item():.4f}, mean={rays_z_after.mean().item():.4f}")
            print(f"[encode_decode debug] outputs['radius'] shape: {outputs['radius'].shape}")
            print(f"[encode_decode debug] outputs['radius'] stats: min={outputs['radius'].min().item():.4f}, max={outputs['radius'].max().item():.4f}, median={outputs['radius'].median().item():.4f}")
            print(f"[encode_decode debug] outputs['depth'] shape: {outputs['depth'].shape}")
            print(f"[encode_decode debug] outputs['depth'] stats: min={outputs['depth'].min().item():.4f}, max={outputs['depth'].max().item():.4f}, median={outputs['depth'].median().item():.4f}")
            print(f"[encode_decode debug] depth formula: depth = rays_z * radius")
            depth_ratio = outputs['depth'].median() / outputs['radius'].median()
            print(f"[encode_decode debug] depth/radius ratio (median): {depth_ratio.item():.4f} (should be close to the mean of rays_z)")
            print(f"[encode_decode debug] === leaving encode_decode ===\n")
    ...
    def load_pretrained(self, model_file):
        ...
        for k, v in dict_model.items():
            if k.startswith("pixel_encoder.backbone."):
                remapped[k] = v
            elif k.startswith("pixel_encoder.") and "fusion_blocks" not in k:
                new_key = k.replace("pixel_encoder.", "pixel_encoder.backbone.", 1)
                remapped[new_key] = v
            else:
                remapped[k] = v
        ...
        pretrained_keys = set(dict_model.keys())

        info = self.load_state_dict(dict_model, strict=False)

        self._new_param_names = set(info.missing_keys) if info.missing_keys else set()
        self._pretrained_param_names = pretrained_keys & model_keys
    ...
    def get_params(self, config):
        """
        Build optimizer parameter groups with differential LR for pretrained vs new parameters.
        """
        has_new_params = hasattr(self, "_new_param_names") and len(self._new_param_names) > 0
        has_pretrained_params = hasattr(self, "_pretrained_param_names") and len(self._pretrained_param_names) > 0

        lr_warmup_factor = config["training"].get("lr_warmup", 1.0)
        base_train_lr = config["training"].get("lr", 0.0)
        final_train_lr = config["training"].get("lr_final", None)

        def build_param_group(params, base_lr, weight_decay, name, lr_scale=1.0):
            if not params:
                return None

            actual_lr = base_lr * lr_scale
            if base_train_lr:
                lr_ratio = actual_lr / base_train_lr
                init_lr = base_train_lr * lr_warmup_factor * lr_ratio
                final_lr = (
                    final_train_lr * lr_ratio
                    if final_train_lr is not None
                    else actual_lr
                )
            else:
                init_lr = actual_lr * lr_warmup_factor
                final_lr = final_train_lr if final_train_lr is not None else actual_lr
            ...
            return {
                ...
                "weight_decay_init": weight_decay,
                "weight_decay_base": weight_decay,
                "weight_decay_final": weight_decay,
                "name": name,
            }

        if has_new_params or has_pretrained_params:
            if is_main_process():
                print("\n" + "=" * 80)
                print("Differential LR configuration (warm-up aware)")
                print("=" * 80)
                print(f"Decoder base LR: {config['training']['lr']}")
                print(f"Encoder base LR: {config['model']['pixel_encoder']['lr']}")
                print("New parameter LR multiplier: 10.0x")
                print("Pretrained parameter LR multiplier: 0.1x")
                print("=" * 80 + "\n")

            param_groups = []

            encoder_base_lr = config["model"]["pixel_encoder"]["lr"]
            encoder_wd = config["training"]["wd"]

            encoder_new_params = []
            encoder_pretrained_params = []

            for name, param in self.pixel_encoder.named_parameters():
                if not param.requires_grad:
                    continue

                full_name = f"pixel_encoder.{name}"
                if full_name in getattr(self, "_new_param_names", set()):
                    encoder_new_params.append(param)
                elif full_name in getattr(self, "_pretrained_param_names", set()):
                    encoder_pretrained_params.append(param)
                else:
                    encoder_new_params.append(param)

            group = build_param_group(
                encoder_new_params,
                encoder_base_lr,
                encoder_wd,
                name="encoder_new",
            ...
            decoder_base_lr = config["training"]["lr"]
            decoder_wd = config["training"]["wd"]
            ...
            for name, param in self.pixel_decoder.named_parameters():
                ...
                full_name = f"pixel_decoder.{name}"
                if full_name in getattr(self, "_new_param_names", set()):
        ...
        default_groups = [*encoder_p, *decoder_p]
        for group in default_groups:
            if "lr_init" not in group:
                if base_train_lr:
                    lr_ratio = group.get("lr", base_train_lr) / base_train_lr
                    group["lr_init"] = base_train_lr * lr_warmup_factor * lr_ratio
                else:
                    group["lr_init"] = group.get("lr", 0.0) * lr_warmup_factor
            if "lr_base" not in group:
                group["lr_base"] = group.get("lr", base_train_lr)
            if "lr_final" not in group and final_train_lr is not None:
                if base_train_lr:
                    lr_ratio = group.get("lr", base_train_lr) / base_train_lr
                    group["lr_final"] = final_train_lr * lr_ratio
                else:
                    group["lr_final"] = final_train_lr
            if "weight_decay_init" not in group:
                group["weight_decay_init"] = group.get(
                    "weight_decay", config["training"].get("wd", 0.0)
                )
            if "weight_decay_base" not in group:
                group["weight_decay_base"] = group.get(
                    "weight_decay", config["training"].get("wd", 0.0)
                )
            if "weight_decay_final" not in group:
                group["weight_decay_final"] = group.get(
                    "weight_decay",
                    config["training"].get("wd_final", group.get("weight_decay", 0.0)),
                )
    ...
    def build(self, config):
        ...
        encoder_name = config["model"]["pixel_encoder"]["name"]
        if encoder_name != "dinov2_vitl14_DSFA":
            raise ValueError(
                f"UniDepthV2DSFA only supports pixel_encoder 'dinov2_vitl14_DSFA', got '{encoder_name}'."
            )
        pixel_encoder = dinov2_vitl14_DSFA(pixel_encoder_config)

        config["model"]["pixel_encoder"]["patch_size"] = (
            14 if "dino" in encoder_name else 16
        )
        ...
        self.fusion_layers = getattr(self.pixel_encoder, "fusion_layers", [])
