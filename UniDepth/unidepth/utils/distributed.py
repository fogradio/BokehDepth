...
def create_local_process_group() -> None:
    ...
    # Debug info.
    if rank == 0:
        print(f"[DEBUG] create_local_process_group: world_size={world_size}, num_gpus={num_workers_per_machine}")

    # Verify that world_size is a multiple of the GPU count.
    if world_size % num_workers_per_machine != 0:
        raise RuntimeError(
            f"World size ({world_size}) must be divisible by number of GPUs per machine ({num_workers_per_machine}). "
            f"Please ensure you launch exactly {num_workers_per_machine} processes (use --nproc_per_node={num_workers_per_machine})."
        )

    num_machines = world_size // num_workers_per_machine
    machine_rank = rank // num_workers_per_machine
