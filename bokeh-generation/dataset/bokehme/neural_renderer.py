#!/usr/bin/env python
# encoding: utf-8
"""Neural rendering networks (ARNet + IUNet) used by the BokehMe pipeline.

Ported from the BokehMe reference implementation
(https://github.com/JuewenPeng/BokehMe). Used in this repo only as a frozen
teacher for generating synthetic bokeh training pairs.
"""
...
class Space2Depth(nn.Module):
    """Inverse of nn.PixelShuffle: pack spatial dims into channels."""
    ...
    def forward(self, x):
        n, c, h, w = x.size()
        unfolded_x = F.unfold(x, self.down_factor, stride=self.down_factor)
        return unfolded_x.view(
            n, c * self.down_factor ** 2, h // self.down_factor, w // self.down_factor
        )


def conv_bn_activation(in_channels, out_channels, kernel_size, stride, padding, use_bn, activation):
    """Conv + (optional BN) + (optional activation), as a Sequential."""
    module = nn.Sequential()
    module.add_module("conv", nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding))
    if use_bn:
        module.add_module("bn", nn.BatchNorm2d(out_channels))
    if activation is not None:
        module.add_module("activation", activation)
    return module


class BlockStack(nn.Module):
    """Stack of residual conv blocks; supports several connection patterns.

    See "Fast and Accurate Image Super-Resolution with Deep Laplacian
    Pyramid Networks" for the connection-mode taxonomy.
    """
    ...
    def __init__(self, channels, num_block, share_weight, connect_mode, use_bn, activation):
        ...
        if share_weight is True:
            block = nn.Sequential(
                conv_bn_activation(channels, channels, 3, 1, 1, use_bn, activation),
                conv_bn_activation(channels, channels, 3, 1, 1, use_bn, activation),
            )
...
class ARNet(nn.Module):
    """Adaptive Rendering Network from BokehMe."""
...
class IUNet(nn.Module):
    """Iterative Upsampling Network from BokehMe."""
    ...
    def forward(self, image, defocus, bokeh_coarse, gamma):
        ...
        if bokeh_coarse.shape[2] != x.shape[2] or bokeh_coarse.shape[3] != x.shape[3]:
            bokeh_coarse = F.interpolate(
                bokeh_coarse, size=(x.shape[2], x.shape[3]), mode="bilinear", align_corners=False
            )
        gamma = torch.ones_like(x[:, :1]) * gamma
